    ...
    , char* s)           //   String to add to trace print message, identifying more about this instance of invocation. Set to NULL if unused.
{
    ...
    if (rc == 1) {
        printf ("WARNING: config_read rc=1, shouldn't happen\n");
    }
    ...
    , char* s)     //   String to add to trace print message, identifying more about this instance of invocation. Set to NULL if unused.
{
    ...
    if (eos_mask            == U32_ZERO && eos_bit            != DEVSTAT_EOS) {
        ERRORS_DETECTED++;
...
{
    if (axi_devsel == FA_QSPI) {
        return "QSPI";
    }

    if (axi_devsel == FA_ICAP) {
        return "ICAP";
    }
...
{
    if (exp_enab == FA_EXP_OFF) {
        return "OFF";
    }

    if (exp_enab == FA_EXP_ON) {
        return "ON ";
    }
...
{
    if (exp_dir == FA_EXP_0123) {
        return "0123";
    }

    if (exp_dir == FA_EXP_3210) {
        return "3210";
    }
...
{
    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SRR) {
        return "SRR   ";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SPICR) {
        return "SPICR ";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SPISR) {
        return "SPISR ";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SPIDTR) {
        return "SPIDTR";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SPIDRR) {
        return "SPIDRR";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_SPISSR) {
        return "SPISSR";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_TXFIFO) {
        return "TXFIFO";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_RDFIFO) {
        return "RDFIFO";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_DGIER) {
        return "DGIER ";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_IPISR) {
        return "IPISR ";
    }

    if (axi_devsel == FA_QSPI && axi_addr == FA_QSPI_IPIER) {
        return "IPIER ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_GIER) {
        return "GIER  ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_ISR) {
        return "ISR   ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_IER) {
        return "IER   ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_WF) {
        return "WF    ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_RF) {
        return "RF    ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_SZ) {
        return "SZ    ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_CR) {
        return "CR    ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_SR) {
        return "SR    ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_WFV) {
        return "WFV   ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_RFO) {
        return "RFO   ";
    }

    if (axi_devsel == FA_ICAP && axi_addr == FA_ICAP_ASR) {
        return "ASR   ";
    }
...
)
{
    ...
    if (resp != FA_WR_RESP_OK) {
        ...
        switch (resp) {
        case FA_WR_RESP_OK     :
            sprintf (s_err, "SUCCESSFUL    ");
            break;

        case FA_WR_RESP_RSVD   :
            sprintf (s_err, "RESERVED      ");
            break;

        case FA_WR_RESP_SLVERR :
            sprintf (s_err, "SLAVE_ERROR   ");
            break;

        case FA_WR_RESP_INVLD  :
            sprintf (s_err, "INVALID SELECT");
            break;

        default                :
            sprintf (s_err, "<UNKNOWN>     ");
        }
...
)
{
    ...
    if (resp != FA_RD_RESP_OK) {
        ...
        case FA_RD_RESP_RSVD   :
            sprintf (s_err, "RESERVED      ");
            break;

        case FA_RD_RESP_SLVERR :
            sprintf (s_err, "SLAVE_ERROR   ");
            break;

        case FA_RD_RESP_INVLD  :
            sprintf (s_err, "INVALID SELECT");
            break;
    ...
    if (TRC_AXI == TRC_ON) {
        printf ("trace    axi_read completion   (rdata h%8x)\n", rdata);
    }
...
{
    if (flash_devsel == SPISSR_SEL_DEV1) {
        return "DEV1";
    }

    if (flash_devsel == SPISSR_SEL_DEV2) {
        return "DEV2";
    }

    if (flash_devsel == SPISSR_SEL_NONE) {
        return "NONE";
    }
...
void fo_wait_for_DTR_FIFO_empty()  // Wait for IPISR[2] to become 1 indicating DTR FIFO has been emptied (all bytes sent to the FLASH)
{
    ...
    if (i == maxloops) {
        ...
        if (debug == 1) {
            printf ("flash_op-fo_wait_for_DTR_FIFO_empty: DTR FIFO is empty after iteration %d, continue\n", i);
        }
...
)
{
    ...
    if ((dir == FO_DIR_XCHG || dir == FO_DIR_RD) || (dir == FO_DIR_WR && FLASH_OP_CHECK == FO_CHK_ON)) {
        ...
        while (remaining_bytes > 0) {
            ...
            if (remaining_bytes >= 4) {   // Read 4 bytes at a time using Byte Expander
                ...
                * (drr_data + i) = (byte) (axi_rdata & 0x000000FF);
                ...
                if (debug == 1) {
                    printf ("flash_op-fo_read_DRR: SPIDRR = h%8X, drr_data[%3d] = h%2X\n",  axi_rdata, i, * (drr_data + i));
                }
        ...
        if (debug == 1) {
            printf ("flash_op-fo_read_DRR: Skipping read of DRR FIFO, returning h00 in all bytes\n");
        }
...
)
{
    ...
    case 1:
        header_array[1] = (byte) (addr & 0x000000FF);                // Cast to byte to reduce from 32 bits to 8 bits
        header_ptr = 2;
        break;

    case 2:
        header_array[1] = (byte) ((addr & 0x0000FF00) >>  8);
        header_array[2] = (byte) (addr & 0x000000FF);
        header_ptr = 3;
        break;

    case 3:
        header_array[1] = (byte) ((addr & 0x00FF0000) >> 16);
    ...
    case 4:
        header_array[1] = (byte) ((addr & 0xFF000000) >> 24);
    ...
    default:
        ERRORS_DETECTED++;   // Shouldn't happen if argument check above is in place
        printf ("(flash_op):  *** ERROR - num_addr (%d) is not in the range 0,1,2,3,4 ***\n", num_addr);
        return;   // ABORT
    }
    ...
    if (debug >= 1) {   // print header_array
        ...
        for (i = 0; i < header_ptr; i++) {
            sprintf (ds, "%s %2.2X ", ds, header_array[i]);
        }
    ...
    if (debug >= 2) {
        printf ("flash_op (debug) - (at A) remaining_total_bytes = %d, remaining_fifo_bytes = %d\n", remaining_total_bytes, remaining_fifo_bytes);
    }
    ...
    while (remaining_total_bytes > 0 && remaining_fifo_bytes > 0) {   // Fill FIFO with as much remaining data as possible
        ...
        if (debug >= 2) {
            printf ("flash_op (debug) - (at B) remaining_total_bytes = %d, remaining_fifo_bytes = %d\n", remaining_total_bytes, remaining_fifo_bytes);
        }
    }
    ...
    while (remaining_total_bytes > 0) {
        if (debug >= 2) {
            printf ("flash_op (debug) - (at C) remaining_total_bytes = %d, remaining_fifo_bytes = %d\n", remaining_total_bytes, remaining_fifo_bytes);
        }
        ...
        if (debug >= 2) {
            printf ("flash_op (debug) - (at D) remaining_total_bytes = %d, remaining_fifo_bytes = %d\n", remaining_total_bytes, remaining_fifo_bytes);
        }
...
void flash_setup (u32 devsel)  // Setup selected FLASH for 9V3 board usage (pass in SPISSR_SEL_DEV1 or SPISSR_SEL_DEV2)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("flash_setup: Start - %s\n", call_args);
    }
...
void fr_wait_for_WRITE_IN_PROGRESS_to_clear (u32 devsel)    // Wait for 'WRITE IN PROGRESS' status bit to return to not busy state
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_wait_for_WRITE_IN_PROGRESS_to_clear: Begin polling for STATUS[0] to become 0 on %s\n", call_args);
    }
    ...
    while (i++ < maxloops && ((rbyte & 0x01) == 0x01)) {     // Wait for IPISR[0] to become 0 indicating write is not in progress
        ...
        if (debug) {
            printf ("fr_wait_for_WRITE_IN_PROGRESS_to_clear: Poll loop iteration %d, rbyte = %2.2X\n", i, rbyte);
        }
    }
    ...
    if (i >= maxloops) {
        ...
        if (TRC_FLASH_CMD) {
            printf ("fr_wait_for_WRITE_IN_PROGRESS_to_clear: (done) Found STATUS[0]=0 (Write In Progress is READY) after %d iterations\n", i);
        }
    }
...
void fw_Reset_Enable (u32 devsel)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Reset_Enable: devsel %s\n", flash_devsel_as_str (devsel));
    }
...
void fw_Reset_Memory (u32 devsel)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Reset_Memory: devsel %s\n", flash_devsel_as_str (devsel));
    }
...
void fw_Enter_4B_Adress_Mode (u32 devsel)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Enter_4B_Address_Mode: devsel %s\n", flash_devsel_as_str (devsel));
    }
...
void fw_Write_Enable (u32 devsel)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Write_Enable: devsel %s\n", flash_devsel_as_str (devsel));
    }
...
byte fr_Enhanced_Volatile_Configuration_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Enhanced_Volatile_Configuration_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Enhanced_Volatile_Configuration_Register: (done) devsel %s, rdata %2.2X\n", flash_devsel_as_str (devsel), rary[0]);
    }
...
void fw_Enhanced_Volatile_Configuration_Register (u32 devsel, byte wdata)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Enhanced_Volatile_Configuration_Register: devsel %s, wdata %2X\n", flash_devsel_as_str (devsel), wdata);
    }
...
byte fr_Extended_Address_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Extended_Address_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Extended_Address_Register: (done) devsel %s, rdata %2.2X\n", flash_devsel_as_str (devsel), rary[0]);
    }
...
void fw_Extended_Address_Register (u32 devsel, byte wdata)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Extended_Address_Register: devsel %s, wdata %2X\n", flash_devsel_as_str (devsel), wdata);
    }
...
byte fr_Status_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Status_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Status_Register: (done) devsel %s, rdata %2.2X\n", flash_devsel_as_str (devsel), rary[0]);
    }
...
void fw_Status_Register (u32 devsel, byte wdata)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Status_Register: devsel %s, wdata %2X\n", flash_devsel_as_str (devsel), wdata);
    }
...
byte fr_Flag_Status_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Flag_Status_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Flag_Status_Register: (done) devsel %s, rdata %2.2X\n", flash_devsel_as_str (devsel), rary[0]);
    }
...
void fw_Clear_Flag_Status_Register (u32 devsel)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Clear_Flag_Status_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
...
u32 fr_Nonvolatile_Configuration_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Nonvolatile_Configuration_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Nonvolatile_Configuration_Register: (done) devsel %s, rdata %2.2X %2.2X\n", flash_devsel_as_str (devsel), rary[0], rary[1]);
    }
...
void fw_Nonvolatile_Configuration_Register (u32 devsel, u32 wdata)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Nonvolatile_Configuration_Register: devsel %s, wdata %2.2X %2.2X\n", flash_devsel_as_str (devsel), wary[0], wary[1]);
    }
...
byte fr_Volatile_Configuration_Register (u32 devsel)
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Volatile_Configuration_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Volatile_Configuration_Register: (done) devsel %s, rdata %2.2X\n", flash_devsel_as_str (devsel), rary[0]);
    }
...
void fw_Volatile_Configuration_Register (u32 devsel, byte wdata)
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_Volatile_Configuration_Register: devsel %s, wdata %2X\n", flash_devsel_as_str (devsel), wdata);
    }
...
void fr_Device_ID_Register (u32 devsel, byte* rdata)   // 20 bytes of read data stored in buffer whose address is passed in
{
    ...
    if (check_TRC_FLASH_CMD()) {
        printf ("fr_Device_ID_Register: devsel %s\n", flash_devsel_as_str (devsel));
    }
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Device_ID_Register: (done) devsel %s, rdata %s\n", flash_devsel_as_str (devsel), ds);
    }
...
void fw_4KB_Subsector_Erase (u32 devsel, u32 addr)  // 3 byte address
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_4KB_Subsector_Erase: devsel %s, addr %4X\n", flash_devsel_as_str (devsel), addr);
    }
...
void fw_64KB_Sector_Erase (u32 devsel, u32 addr)  // 3 byte address
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fw_64KB_Sector_Erase: devsel %s, addr %4X\n", flash_devsel_as_str (devsel), addr);
    }
...
void fr_Read (u32 devsel, u32 addr, int num_bytes, byte* rary)  // 3 byte address
{
    ...
    for (i = 0; i < num_bytes; i++) {
        wary[i] = 0x00;    // Clear write data
    }

    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Read: devsel %s, addr %4X, num_bytes %d\n", flash_devsel_as_str (devsel), addr, num_bytes);
    }
...
void fw_Page_Program (u32 devsel, u32 addr, int num_bytes, byte* wary)  // 3 byte address
{
    ...
    if (TRC_FLASH_CMD == TRC_ON) {
        printf ("fr_Page_Program: devsel %s, addr %4X, num_bytes %d\n", flash_devsel_as_str (devsel), addr, num_bytes);
    }
